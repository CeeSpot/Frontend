...
<!-- —————————————↓HTML————————————————— -->
<template>
  <div class="vue-masonry-gallery-container"
    ...
    <div class="vue-masonry-gallery-scroll" ref="scrollEl">
      ...
      <div class="vue-masonry-gallery"
           :style="isMobile? '' :{width: colWidth*cols+'px',left:'50%', marginLeft: -1*colWidth*cols/2 +'px'}">
        <div class="img-box" v-for="(v,i) in imgsArr_c" v-if="i < currentlyVisible" :class="[cardAnimationClass]"
          ...
          <component class="img-inner-box" :is="isRouterLink  && linkRange=='card' ? 'router-link' : 'alink'"
            ...
            <component class="img-wraper" v-if="v[srcKey]"
                       :is="isRouterLink && linkRange=='img' ? 'router-link' :'alink'"
                       :to="linkRange=='img' ? v[hrefKey] : false "
                       :style="{width:imgWidth_c + 'px',height:v._height ? v._height+'px':false}"><img
    ...
    <div id="footer">
      <button class="loadMore" @click="loadMore" v-if="currentlyVisible < imgsArr_c.length">Load more items</button>
      <p class="copyright">VajraCode 2018</p>
    </div>
  </div>
</template>
